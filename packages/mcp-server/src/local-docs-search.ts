...
const EMBEDDED_METHODS: MethodEntry[] = [
  ...
  },
  {
    ...
    response:
      "{ id: string; agentId: string; cost: number; createdAt: string; inputTokens: number; latencyMs: number; outputTokens: number; status: 'success' | 'error' | 'filtered' | 'rate_limited' | 'balance_insufficient'; errorMessage?: string; inboundMessageId?: string; knowledgeChunksUsed?: number; responseMessageId?: string; responseText?: string; }",
    markdown:
      "## list\n\n`client.senders.agent.executions.list(senderId: string, cursor?: string, limit?: number, status?: 'success' | 'error' | 'filtered' | 'rate_limited' | 'balance_insufficient'): { id: string; agentId: string; cost: number; createdAt: string; inputTokens: number; latencyMs: number; outputTokens: number; status: agent_execution_status; errorMessage?: string; inboundMessageId?: string; knowledgeChunksUsed?: number; responseMessageId?: string; responseText?: string; }`\n\n**get** `/v1/senders/{senderId}/agent/executions`\n\nList recent agent executions with pagination.\n\nAn execution is one inbound message answered by the agent, so this covers the messaging channels only. Voice calls are never listed here regardless of how many the agent handled. Use `GET /v1/calls` (and `GET /v1/calls/{callId}` for the transcript) for voice.\n\n### Parameters\n\n- `senderId: string`\n\n- `cursor?: string`\n\n- `limit?: number`\n\n- `status?: 'success' | 'error' | 'filtered' | 'rate_limited' | 'balance_insufficient'`\n  Status of an agent execution.\n\n### Returns\n\n- `{ id: string; agentId: string; cost: number; createdAt: string; inputTokens: number; latencyMs: number; outputTokens: number; status: 'success' | 'error' | 'filtered' | 'rate_limited' | 'balance_insufficient'; errorMessage?: string; inboundMessageId?: string; knowledgeChunksUsed?: number; responseMessageId?: string; responseText?: string; }`\n\n  - `id: string`\n  - `agentId: string`\n  - `cost: number`\n  - `createdAt: string`\n  - `inputTokens: number`\n  - `latencyMs: number`\n  - `outputTokens: number`\n  - `status: 'success' | 'error' | 'filtered' | 'rate_limited' | 'balance_insufficient'`\n  - `errorMessage?: string`\n  - `inboundMessageId?: string`\n  - `knowledgeChunksUsed?: number`\n  - `responseMessageId?: string`\n  - `responseText?: string`\n\n### Example\n\n```typescript\nimport Zavudev from '@zavudev/sdk';\n\nconst client = new Zavudev();\n\n// Automatically fetches more pages as needed.\nfor await (const agentExecution of client.senders.agent.executions.list('senderId')) {\n  console.log(agentExecution);\n}\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    response:
      '{ execution: { id: string; agentId: string; cost: number; createdAt: string; inputTokens: number; latencyMs: number; outputTokens: number; status: agent_execution_status; errorMessage?: string; inboundMessageId?: string; knowledgeChunksUsed?: number; responseMessageId?: string; responseText?: string; }; }',
    markdown:
      "## retrieve\n\n`client.senders.agent.executions.retrieve(senderId: string, executionId: string): { execution: agent_execution; }`\n\n**get** `/v1/senders/{senderId}/agent/executions/{executionId}`\n\nFetch full details for one execution — including `errorMessage`, `errorCode`, and `responseText`. Use this to debug failures surfaced by the list endpoint.\n\n### Parameters\n\n- `senderId: string`\n\n- `executionId: string`\n\n### Returns\n\n- `{ execution: { id: string; agentId: string; cost: number; createdAt: string; inputTokens: number; latencyMs: number; outputTokens: number; status: agent_execution_status; errorMessage?: string; inboundMessageId?: string; knowledgeChunksUsed?: number; responseMessageId?: string; responseText?: string; }; }`\n\n  - `execution: { id: string; agentId: string; cost: number; createdAt: string; inputTokens: number; latencyMs: number; outputTokens: number; status: 'success' | 'error' | 'filtered' | 'rate_limited' | 'balance_insufficient'; errorMessage?: string; inboundMessageId?: string; knowledgeChunksUsed?: number; responseMessageId?: string; responseText?: string; }`\n\n### Example\n\n```typescript\nimport Zavudev from '@zavudev/sdk';\n\nconst client = new Zavudev();\n\nconst execution = await client.senders.agent.executions.retrieve('executionId', { senderId: 'senderId' });\n\nconsole.log(execution);\n```",
    perLanguage: {
      typescript: {
