...
export interface ClientOptions {
  /**
   * Defaults to process.env['ZAVUDEV_API_KEY'].
   */
   ...
   *
   * Defaults to process.env['ZAVUDEV_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['ZAVUDEV_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Zavudev API.
 */
export class Zavudev {
  apiKey: string;
   ...
   * API Client for interfacing with the Zavudev API.
   *
   * @param {string | undefined} [opts.apiKey=process.env['ZAVUDEV_API_KEY'] ?? undefined]
   * @param {string} [opts.baseURL=process.env['ZAVUDEV_BASE_URL'] ?? https://api.zavu.dev] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('ZAVUDEV_BASE_URL'),
    apiKey = readEnv('ZAVUDEV_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.ZavudevError(
        "The ZAVUDEV_API_KEY environment variable is missing or empty; either provide it, or instantiate the Zavudev client with an apiKey option, like new Zavudev({ apiKey: 'My API Key' }).",
      );
    }
    ...
    const options: ClientOptions = {
      apiKey,
      ...opts,
      baseURL: baseURL || `https://api.zavu.dev`,
    };

    this.baseURL = options.baseURL!;
    this.timeout = options.timeout ?? Zavudev.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('ZAVUDEV_LOG'), "process.env['ZAVUDEV_LOG']", this) ??
      defaultLogLevel;
    ...
    const customHeadersEnv = readEnv('ZAVUDEV_CUSTOM_HEADERS');
    if (customHeadersEnv) {
  ...
  #baseURLOverridden(): boolean {
    return this.baseURL !== 'https://api.zavu.dev';
  }
  ...
  static ZavudevError = Errors.ZavudevError;
